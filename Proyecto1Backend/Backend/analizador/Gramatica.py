from Errores import TokenError
...
def p_operacionLocalsen(p):
    ...
    p[0] = OperacionSen(p[3],p.lineno(1),buscar_columna(p.slice[1]))

def p_operacionLocalcos(p):
    'local    : COS PARIZQ operacion PARDER '
    p[0] = OperacionCos(p[3],p.lineno(1),buscar_columna(p.slice[1]))

def p_operacionLocalTan(p):
    'local    : TAN PARIZQ operacion PARDER '
    p[0] = OperacionTan(p[3],p.lineno(1),buscar_columna(p.slice[1]))

def p_operacionLocalLog10(p):
    'local    : LOG10 PARIZQ operacion PARDER '
    p[0] = OperacionLog10(p[3],p.lineno(1),buscar_columna(p.slice[1]))

def p_operacionLocalLog(p):
    'local    : LOG PARIZQ operacion COMA operacion PARDER '
    p[0] = OperacionLog(p[3],p[5],p.lineno(1),buscar_columna(p.slice[1]))

def p_operacionLocalSqrt(p):
    'local    : SQRT PARIZQ operacion PARDER '
    p[0] = OperacionSQRT(p[3],p.lineno(1),buscar_columna(p.slice[1]))
