...
#logica
    ...
    def procesar_operacionLog(self,operacion,ts):
        ...
        try:
            ...
            nuevo = err.TokenError("Semantico","Operacion desconocida", operacion.linea)
            self.lst_errores.append(nuevo)
...
#valores
    def procesar_valor(self, expresion,ts):
        if isinstance(expresion, OperacionNumero):
            ...
            if ts.existe(expresion.id) == True:     
...
#sentencia For
    ...
    def procesar_for(self,instr,ts):
        ...
        if isinstance(instr.condicional, condicionalSimple):
            ...
            op1 = self.procesar_valor(instr.condicional.rangoizq,ts)
            op2 = self.procesar_valor(instr.condicional.rangoder,ts)
                        ...
                        if func.parametros != None:
                            if len(func.parametros) == len(inst.parametros):
                                ...
                                while cont < len(func.parametros):
                                    simbololocal = TS.Simbolo(func.parametros[cont].idparametro,self.procesar_valor(inst.parametros[cont],ts),func.parametros[cont].tipo,func.parametros[cont].linea,func.parametros[cont].columna)
